            ...
            <Popover open={open} onOpenChange={setOpen}>
                ...
                <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" aria-expanded={open} className="w-full justify-start text-left font-normal">
                        {value ? items.find((framework) => framework.value.toString() === value)?.label : `Select ${placeholder}...`}
                        ...
                        <CommandList>
                            ...
                            <CommandGroup>
                                {items.map((framework) => (
                                    ...
                                    >
                                        {framework.label}
                                        <Check className={cn('ml-auto', value === framework.value.toString() ? 'opacity-100' : 'opacity-0')} />
                                    </CommandItem>
                                ))}
                            </CommandGroup>
                        </CommandList>
